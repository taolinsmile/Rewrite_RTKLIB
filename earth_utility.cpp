...
double elmin_mask = 15 * PI / 180;
double ERR_BRDCI = 0.5;
double ERR_CBIAS = 0.3;
double ERR_SAAS = 0.3;
double REL_HUMI = 0.7;
double Error_std[5] = { 100,0.003,0.003,0,1 };
const double maxgdop = 30.0;
const double Ls_res_threshold = 100.0;
const short maxout = 100;
const short minlock = 0;
const short minfix = 100;
const int optmindropsats = 10;
const int optminholdsats = 5;
...
const double VAR_POS = pow(30,2);
const double VAR_VEL = pow(10,2);
const double VAR_ACC = pow(10,2);
const double thresar[8] = { 3.0,0.25,0.0,1E-5,1E-4,0.0,0.0,0.0 };
const double prn[6] = { 1E-4,1E-3,1E-4,1,0.25,0.0 };
const double optstd[3] = { 30.0,0.03,0.3 };
const int NSATGPS = 32;
const int NSATCMP = 35;
const double sclkstab = 5E-12;
const double opterr[5] = { 100.0,0.003,0.003,0.0,1.0 };
const double opteratio[3] = { 300.0,300.0,100.0 };
const double optthresar[8] = { 3.0,0.1,0.25,0.1,0.05,0.0,0.0,0.0 };
const int optminfixsats = 4;
const double varholdamb = 1;
const double optmaxino = 30.0;
/*坐标转换相关函数*/
...
double varerr(double el) {
	double fact, varr;
	fact = 1.0;
	varr = pow(Error_std[0], 2)*(pow(Error_std[1], 2) + pow(Error_std[2], 2) / sin(el));
	//先不考虑双频的事，只考虑单频

...
void OBS_SAT::detslp_ll(char * slip, char * half,  double tt) {
	...
	*half = (base.Sget_LLI(0) & 2) ? 0 : (rover.Sget_LLI(0) & 2) ? 0 : 1;// 这个应该是指示是否半周有效
	...
	if (((basepreviousLLI & 2) && !(base.Sget_LLI(0) & 2)) || (!(basepreviousLLI & 2 & 2) && (base.Sget_LLI(0) & 2))) {
		baseslip |= 1;
	}// slip 低4位 是一个标志，只统计是否发生周跳，不管是整周还是半周。不关心。
	if (((rcvpreviousLLI & 2) && !(rover.Sget_LLI(0) & 2)) || (!(rcvpreviousLLI & 2 & 2) && (rover.Sget_LLI(0) & 2))) {
		roverslip |= 1;
...
void OBS_SAT::calsdresvar(double bl) {
	...
	c = opterr[3] * bl / 1E4;
	d = Cspeed * sclkstab*dt;
	double factL = 1.0,factP = opteratio[0];
	a = factL * opterr[1];
	b = factL * opterr[2];
...
int Station_OBS::zdres(const vector<double > & posecef, const vector<double > & posllh) {
	if (posecef.empty() || norm_vector(posecef, 0, 3) <= 0.0)
		return 0;
	geodist(posecef, SAT_POS_VEL);
	satazel(posllh);
	unused_flag = 0;
	if (r_azel_los[0] < 0 || r_azel_los[2] < elmin_mask)//
		unused_flag = 1;
...
double  Station_OBS::cal_P(double *P, string mode, const vector<int> &fre,char constllation,double TGD) {
	if (mode == "single") {
		*P = Sget_P(fre[0]);
		...
		if (constllation == 'G') // RTKLIB没有校正北斗TGD
			*P = *P - TGD * Cspeed;
		return gamma;
	}
	return 0.0;
...
void Station_OBS::update_razellos(const vector<double > &sysclk_bias, const vector<double> &receiver_pos_ecef,
	...
	geodist(receiver_pos_ecef, SAT_POS_VEL);
	...
	if (r_azel_los[0] < 0 || r_azel_los[2] < elmin_mask)//
		unused_flag = 1;
	if (unused_flag || sat_svh < 0 || fde_mask)
		return;
	count++;
	double P;
	...
	P_zres = P - (r_azel_los[0] + sysclk_bias[0] - Cspeed * SATCLK_BIAS_DRIFT[0] + dion + dtrop);
...
void Station_OBS::geodist(const vector<double> &receiver_pos_ecef,const vector<double> & SAT_POS_VEL) {
	...
	if (rr < RE_WGS84) {
		...
		divide_vector(r_azel_los, 3, 3, r);
